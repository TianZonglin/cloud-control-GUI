...
float skelft2DMakeBoundary(const unsigned char* input, int xm, int ym, int xM, int yM, float* param, int size, short iso=1, bool thr_upper=true);
float skelft2DMakeBoundary(const float* input, int xm, int ym, int xM, int yM, float* param, int size, float iso=1, bool thr_upper=true);
...
void initShepard(const PointCloud& cloud, const float* point_data);
//Initialize the computation of a (simple) Shepard-like interpolation of the scalar signal point_data[] over the locations in 'cloud'.
